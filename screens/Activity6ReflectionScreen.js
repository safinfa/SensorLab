...
export default function Activity6ReflectionScreen({ navigation, route }) {
  const { teamName, prediction, results } = route?.params || {};
  const [reflection, setReflection] = useState('');
  const [isSpeaking, setIsSpeaking] = useState(false);
  const [submitting, setSubmitting] = useState(false);

  const reflectionPrompt = 'Time to reflect! Think about your reaction time results. Which hand was faster? Were you surprised by your results? Write your thoughts below.';
  ...
  const avgPhase1 = getAvg(results?.phase1);
  const avgPhase2 = getAvg(results?.phase2);
  const avgPhase3 = getAvg(results?.phase3);
  const totalScore = Math.round((avgPhase1 + avgPhase2) / 2);
    ...
    <LinearGradient colors={['#4a90d9', '#1a3a5c']} style={styles.gradient}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.summaryBox}>
          ...
          <Text style={styles.resultRow}>Phase 1 (Dominant): {avgPhase1} ms</Text>
          <Text style={styles.resultRow}>Phase 2 (Non-Dominant): {avgPhase2} ms</Text>
          <Text style={styles.resultRow}>Phase 3 (Tracing): {avgPhase3}% accuracy</Text>
        </View>

        <View style={styles.questionsBox}>
          <Text style={styles.sectionTitle}>Think About It:</Text>
          <Text style={styles.question}>• Which hand reacted faster and by how much?</Text>
          <Text style={styles.question}>• Did your reaction time improve across attempts?</Text>
          <Text style={styles.question}>• Were your results close to your prediction?</Text>
          <Text style={styles.question}>• What could affect your reaction time in real life?</Text>
        </View>
